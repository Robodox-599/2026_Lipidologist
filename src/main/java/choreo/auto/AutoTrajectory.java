...
public class AutoTrajectory {
  ...
  private void cmdExecute() {
    ...
    if (error > maxErrorMeters
        || (!activeTimer.isRunning() && error > recoverErrorMeters)) {
      activeTimer.stop();
    } else {
      activeTimer.start();
    }
  ...
  public Trigger atTime(double timeSinceStart) {
    ...
    // The timer should never exceed the total trajectory time so report this as a
    // warning
  ...
  public Trigger atTime(String eventName) {
    ...
    for (var event : trajectory.getEvents(eventName)) {
      ...
      // with having it all be 1 trigger that just has a list of times and checks each
      // one each
      // cycle
      // or something like that. If choreo starts proposing memory issues we can look
      // into this.
      trig = trig.or(atTime(event.timestamp));
      foundEvent = true;
    }

    // The user probably expects an event to exist if they're trying to do something
    // at that event,
  ...
  public Trigger atPose(String eventName, double toleranceMeters, double toleranceRadians) {
    ...
    for (var event : trajectory.getEvents(eventName)) {
      // This could create a lot of objects, could be done a more efficient way
      // with having it all be 1 trigger that just has a list of possess and checks
      // each one each
  ...
  public Trigger atTranslation(String eventName, double toleranceMeters) {
    ...
    for (var event : trajectory.getEvents(eventName)) {
      // This could create a lot of objects, could be done a more efficient way
      // with having it all be 1 trigger that just has a list of poses and checks each
      // one each
      // cycle or something like that.
      // If choreo starts showing memory issues we can look into this.
