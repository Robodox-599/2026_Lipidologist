...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
  ...
  private void applyStates() {
    switch(currentState){
      ...
      case TELEOP_DRIVE:
        setControl(drive
          .withVelocityX(driver.getLeftX() * TunerConstants.MaxSpeed)
          .withVelocityY(driver.getLeftY() * TunerConstants.MaxSpeed)
          .withRotationalRate(driver.getRightX()* TunerConstants.MaxSpeed)
      );

      DogLog.log("Drive/Speed", Math.sqrt(Math.pow((driver.getLeftX() * TunerConstants.MaxSpeed), 2) + Math.pow((driver.getRightX() * TunerConstants.MaxSpeed), 2)));
