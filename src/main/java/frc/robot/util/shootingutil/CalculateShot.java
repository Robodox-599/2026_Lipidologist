...
public class CalculateShot {
  public record AdjustedShot(
      Rotation2d targetRotation, double shootSpeed, double hoodAngle, double flightTime) {}

  static final int maxGoalPositionIterations = 5;
  static final double LOOKAHEAD_TIME = 0.05;
  static final double fuelDragCoefficient = 0.1;

  public static AdjustedShot calculateHubAdjustedShot(
      Pose2d robotPose,
    ...
    for (int i = 0; i < maxGoalPositionIterations; i++) {
      adjustedHubTranslation =
          new Translation2d(
              hubTranslation.getX()
                  - (fieldRelativeRobotVelocity.vxMetersPerSecond
                      * flightTime), // vt + 0.5at^2, acceleration compensation may not be needed
    ...
    Translation2d swerveLookAheadTranslation =
        robotTranslation.plus(
            new Translation2d(
                fieldRelativeRobotVelocity.vxMetersPerSecond * LOOKAHEAD_TIME,
    ...
    Rotation2d targetRotation =
        Rotation2d.fromRadians(
            Math.atan2(
                robotTranslation.getY() - adjustedHubTranslation.getY(),
    ...
    DogLog.log(
        "ShotCalculator/AdjustedHubTarget",
        new Pose2d(adjustedHubTranslation.getX(), adjustedHubTranslation.getY(), new Rotation2d()));
    return adjustedShot;
  }

  public static AdjustedShot calculateAllianceZoneAdjustedShot(
      Pose2d robotPose,
      ChassisSpeeds fieldRelativeRobotVelocity,
      ChassisAccelerations fieldRelativeRobotAcceleration) {
    Translation2d robotTranslation = robotPose.getTranslation();
    Translation2d allianceZoneTarget =
        new Translation2d(
            AllianceFlipUtil.applyX(FieldConstants.LinesVertical.starting - 1),
    ...
    Rotation2d targetRotation =
        Rotation2d.fromRadians(
            Math.atan2(
                robotTranslation.getY() - allianceZoneTarget.getY(),
    ...
    DogLog.log("ShotCalculator/AdjustedShot", adjustedShot);
    DogLog.log(
        "ShotCalculator/AllianceZoneTarget",
        new Pose2d(allianceZoneTarget.getX(), allianceZoneTarget.getY(), new Rotation2d()));
    return adjustedShot;
  }
  ...
  public static AdjustedShot calculateManualShot() {
    ...
    //             hubTranslation.getX() - (fieldRelativeRobotVelocity.vxMetersPerSecond *
    // flightTime), // vt + 0.5at^2, acceleration compensation may not be needed
    //             hubTranslation.getY() - (fieldRelativeRobotVelocity.vyMetersPerSecond *
    // flightTime));
    //     double newDistance = robotTranslation.getDistance(adjustedHubTranslation);
    //     double newFlightTime = applyFuelDragCoefficient(ShotData.getHubFlightTime(newDistance));
    ...
    //         .plus(new Translation2d(fieldRelativeRobotVelocity.vxMetersPerSecond *
    // LOOKAHEAD_TIME,
    //                 fieldRelativeRobotVelocity.vyMetersPerSecond * LOOKAHEAD_TIME));
    ...
    Rotation2d targetRotation =
        Rotation2d.fromRadians(
            Math.atan2(
                robotTranslation.getY() - hubTranslation.getY(),
    ...
    DogLog.log(
        "ShotCalculator/AdjustedHubTarget",
        new Pose2d(hubTranslation.getX(), hubTranslation.getY(), new Rotation2d()));
    return adjustedShot;
  }
