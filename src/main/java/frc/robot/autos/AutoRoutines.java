...
public class AutoRoutines {
    ...
    public AutoRoutine leftAutoRoutine() {
        ...
        LMIDtoLTROUGH.done()
                .onTrue(LTROUGHtoDEPOT.cmd()
                        .alongWith(superstructure.setWantedSuperStateCommand(WantedSuperState.SHOOT_HUB)));

        LTROUGHtoDEPOT.done()
                .onTrue(DEPOT_INTAKE.cmd().alongWith(superstructure.setWantedSuperStateCommand(WantedSuperState.IDLE)));

        DEPOT_INTAKE.done().onTrue(superstructure.setWantedSuperStateCommand(WantedSuperState.SHOOT_HUB));

        DEPOT_INTAKE.recentlyDone().and(new Trigger(() -> HubShiftUtil.getShiftInfo().remainingTime() < 5.0))
                .onTrue(superstructure.setWantedSuperStateCommand(WantedSuperState.CLIMB));
